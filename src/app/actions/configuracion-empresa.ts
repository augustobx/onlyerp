...
export async function getDatosEmpresa() {
  ...
  let config = await prisma.empresaConfig.findUnique({ where: { tenantId: tenant.id } });
  if (!config) {
    config = await prisma.empresaConfig.create({
      data: {
        tenantId: tenant.id,
        razon_social: tenant.nombre,
        nombre_fantasia: tenant.nombre,
      },
    });
  }
  return config;
...
// Guarda los datos de la empresa desde la pantalla de configuración
