...
export async function getClientesDeudores(filtros?: {
  ...
  try {
    ...
    let whereVentas: any = { tenantId: tenant.id, estado_pago: { in: ["PENDIENTE", "PARCIAL"] } };
    ...
    let dataFiltrada = clientes.map((c) => ({
      ...
      ventas_vencidas: c.ventas.filter(
        (v) => v.fecha_vencimiento_cc && new Date(v.fecha_vencimiento_cc) < new Date()
      ).length,
    }));
...
export async function getFichaCuentaCorriente(clienteId: number) {
  try {
    ...
    const cliente = await prisma.cliente.findFirst({
      where: { id: clienteId, tenantId: tenant.id },
    });

    const ventasPendientes = await prisma.venta.findMany({
      where: { tenantId: tenant.id, clienteId, estado_pago: { in: ["PENDIENTE", "PARCIAL"] } },
...
export async function registrarPagoCC(data: {
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const montoEfectivo = data.monto - montoDescuento;
      const montoAplicado = data.monto;
      ...
      await tx.movimientoCaja.create({
        data: {
          ...
          metodo_pago: data.metodo_pago as any,
          monto: montoEfectivo,
          descripcion: `Abono CC (Fac. #${venta.numero_comprobante})${
            descPorcentaje > 0 ? ` - Desc. ${descPorcentaje}%` : ""
...
export async function recalcularVentaVencida(ventaId: number) {
  try {
    ...
    const resultado = await prisma.$transaction(async (tx) => {
      const venta = await tx.venta.findFirst({
        where: { id: ventaId, tenantId: tenant.id },
        include: {
          listaPrecio: true,
      ...
      await tx.venta.update({
        ...
        data: {
          ...
          notas_venta: `${venta.notas_venta || ""}\n[Sistema]: Precio actualizado por inflación el ${new Date().toLocaleDateString(
            "es-AR"
          )}.`,
        },
      });
