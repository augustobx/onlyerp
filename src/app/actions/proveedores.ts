...
export async function guardarProveedor(formData: FormData) {
  try {
    ...
    if (error.code === "P2002") return { success: false, error: "Ya existe un proveedor con ese nombre en tu empresa." };
    ...
    await prisma.proveedorListaPrecio.upsert({
      ...
      create: {
        ...
        margen_personalizado:
          margen_personalizado !== null && !isNaN(Number(margen_personalizado)) ? Number(margen_personalizado) : null,
        descuento_personalizado:
          descuento_personalizado !== null && !isNaN(Number(descuento_personalizado))
            ? Number(descuento_personalizado)
            : null,
      },
      update: {
        margen_personalizado:
...
export async function guardarMarca(formData: FormData) {
  try {
    ...
    if (error.code === "P2002") return { success: false, error: "Esta marca ya existe para este proveedor en tu empresa." };
...
// ==========================================
// 4. ACTUALIZACIÓN MASIVA FLEXIBLE
// ==========================================
export async function actualizarPreciosMasivos(
  proveedorIdOrOptions:
    | number
    | {
        destino?: "COSTO_BASE" | number;
  ...
  try {
    ...
    let whereProducto: any = { tenantId: tenant.id };
    ...
    if (targetListaId) {
      const listaObj = await prisma.listaPrecio.findFirst({
        where: { id: targetListaId, tenantId: tenant.id },
      });
      ...
      await prisma.$transaction(async (tx) => {
        for (const prod of productos) {
          ...
          const margenActual =
            pivot?.margen_personalizado !== null && pivot?.margen_personalizado !== undefined
    ...
    await prisma.$transaction(async (tx) => {
      for (const prod of productos) {
        ...
        const porcentajeCambio =
          prod.precio_costo > 0 ? Number((((nuevoCosto - prod.precio_costo) / prod.precio_costo) * 100).toFixed(2)) : 0;
