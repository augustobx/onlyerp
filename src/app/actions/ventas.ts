...
export async function buscarProductos(query: string) {
  ...
  return productos.map((p) => {
    ...
    const stockComprometido = p.detalles_pedido
      ? p.detalles_pedido.reduce((acc: any, current: any) => acc + current.cantidad, 0)
      : 0;
    return {
      ...p,
  ...
  try {
    ...
    const resultado = await prisma.$transaction(
      async (tx) => {
        ...
        if (!cajaAbierta) {
          throw new Error("CAJA_CERRADA: No puede facturar sin abrir la caja diaria en la sucursal activa.");
        }
        ...
        const clienteData = await tx.cliente.findFirst({
          where: { id: data.clienteId, tenantId: tenant.id },
        });
        if (!clienteData) throw new Error("CLIENTE_NO_ENCONTRADO: El cliente asignado no existe.");
        ...
        if (tieneCuentaCorriente && clienteData.limite_credito !== null && clienteData.limite_credito > 0) {
          ...
          if (deudaAcumulada + montoCuentaCorriente > clienteData.limite_credito) {
            throw new Error(
              `LIMITE_CREDITO_EXCEDIDO: Intentás cargar $${montoCuentaCorriente.toFixed(2)}, pero este cliente ya debe $${deudaAcumulada.toFixed(2)} y su tope autorizado es $${clienteData.limite_credito.toFixed(2)}.`
            );
          }
        }
        ...
        if (montoSaldoAFavor > 0) {
          const movimientos = await tx.movimientoCuentaCorriente.findMany({
            where: { tenantId: tenant.id, clienteId: data.clienteId },
          });
          ...
          if (montoSaldoAFavor > saldoAFavorActual) {
            throw new Error(
              `SALDO_INSUFICIENTE: Intentás usar $${montoSaldoAFavor.toFixed(2)} de Saldo a Favor, pero el cliente solo tiene $${saldoAFavorActual.toFixed(2)} disponible.`
            );
          }
        }
        ...
        const nuevaVenta = await tx.venta.create({
          data: {
            ...
            metodo_pago: pagos.length === 1 ? pagos[0].metodo_pago : "CONTADO",
            ...
            detalles: {
              create: await Promise.all(
                data.carrito.map(async (item: any) => {
                  const prod = await tx.producto.findFirst({
                    where: { id: item.productoId, tenantId: tenant.id },
        ...
        // 4. ACTUALIZAR NÚMERO DE FACTURA
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      const venta = await tx.venta.findFirst({
        where: { id: data.ventaId, tenantId: tenant.id },
      });
      if (!venta) throw new Error("Venta no encontrada");

      const depositoDevolucion = venta.depositoOrigenId;
      if (!depositoDevolucion) throw new Error("Esta venta no tiene depósito origen registrado.");
      ...
      for (const item of data.itemsDevueltos) {
        if (item.cantidad > 0) {
          ...
          if (item.cantidad > disponibleRestante) {
            throw new Error(`Intentás devolver más unidades de las disponibles.`);
          }
      ...
      if (data.metodoReembolso === "CAJA") {
        const cajaAbierta = await tx.cajaDiaria.findFirst({
          where: { tenantId: tenant.id, estado: "ABIERTA", sucursalId: venta.sucursalId },
        });
        if (!cajaAbierta) throw new Error("Debe tener la caja abierta en la sucursal.");
        ...
        await tx.movimientoCaja.create({
          data: {
            ...
            descripcion: `Devolución s/ Factura #${venta.numero_comprobante}`,
            ventaId: venta.id,
          },
        });
      ...
      const selloDevolucion = `\n\n[SISTEMA - DEVOLUCIÓN EL ${new Date().toLocaleDateString(
        "es-AR"
